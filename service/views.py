...
class MainView(View):
    ...
    def get(self, request):
        ...
        logger.debug("Got request for key {}".format(key))
        # TODO input validation
        redis_client = redis.Redis(connection_pool=self.pool)
        logger.debug("Checking if key {} is in cache".format(key))
        hash = redis_client.get(key)
        if hash:
            logger.debug("Key {} is in cache. Value: {}".format(key, hash))
            return JsonResponse({
                "hash": hash
            })
        else:
            ...
            if not redis_client.sismember("in_progress", key):
                logger.debug("Task for key {} is not running. "
                             "Starting".format(key))
